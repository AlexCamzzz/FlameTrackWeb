...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" (click)="close.emit()">
      <div class="bg-surface border border-border/20 rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-slide-up" (click)="$event.stopPropagation()">
        <div class="p-6 border-b border-white/5 flex justify-between items-center bg-black/10">
          <h2 class="text-xl font-bold">Set Monthly Budget</h2>
        ...
        <form (ngSubmit)="onSubmit()" class="p-8 space-y-6">
          <div>
            ...
            <button type="button" (click)="isPickerOpen = true" 
              ...
              <ng-template #noCat>
                <span class="text-subtle font-bold italic">Select category...</span>
              </ng-template>
          ...
          <div>
            <label class="block text-xs font-black text-subtle uppercase tracking-widest mb-2 ml-1">Monthly Limit (MXN)</label>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-black text-subtle uppercase tracking-widest mb-2 ml-1">Month</label>
              <input type="number" name="month" [(ngModel)]="model.month" required min="1" max="12"
                class="w-full bg-black/20 border border-white/10 rounded-2xl px-4 py-3 text-white focus:outline-none focus:border-primary transition-all shadow-inner">
            </div>
            <div>
              <label class="block text-xs font-black text-subtle uppercase tracking-widest mb-2 ml-1">Year</label>
          ...
          <div class="pt-4 flex flex-col space-y-3">
            <button type="submit" [disabled]="isSubmitting || !model.categoryId" class="w-full bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 text-white rounded-2xl py-4 font-black transition-all disabled:opacity-50 shadow-xl shadow-primary/20">
              {{ isSubmitting ? 'ESTABLISHING...' : 'SAVE BUDGET' }}
            </button>
